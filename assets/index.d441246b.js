...
`;function st({className:e}){return t("div",{id:"",className:e})}const it=m(st)`
  ...
  width: 0px !important;
`;function at({className:e}){const n=l.exports.useContext(w);l.exports.useEffect(()=>{window.onload=()=>{const o=document.querySelectorAll("#___gcse_0 .gsc-results-wrapper-nooverlay")[0],a={childList:!1,attributes:!0,subtree:!1};function c(){r()}new MutationObserver(c).observe(o,a)}},[]);let r=()=>{setTimeout(()=>{let o=document.querySelectorAll(".gs-title");o&&o.forEach((a,c)=>{if(a.innerText.includes("Lyrics")){let i=a.innerText.replace("Lyrics | Musixmatch"," ");a.innerText=i;const p={artistName:i.split("-")[0],songtName:i.split("-")[1]};a.addEventListener("click",f=>{n.getLines(p)})}else a.innerText.includes("Lyrics")||a.parentElement.parentElement.parentElement.className.includes("gsc-webResult")&&a.parentElement.parentElement.parentElement.remove()})},50)};function s(o){let a=document.querySelector("#gsc-i-id1");if(a)if(o.target.value.length<=1){let c=document.querySelector(".gsst_a");c&&c.dispatchEvent(new Event("click"))}else{a.value=o.target.value;let c=document.querySelectorAll(".gsc-search-box button")[0];c?setTimeout(()=>{c.dispatchEvent(new Event("click"))},100*Math.floor(Math.random()*4)):console.error("no gsc loaded, try reload the page")}else console.error("no gsc loaded, try reload the page")}return x("div",{id:"",className:e,children:[t(ve,{id:"outlined-search",label:"\u05D7\u05E4\u05E9 \u05E9\u05D9\u05E8",type:"search",className:"main-input",onChange:s,autoFocus:!1,autoComplete:"off"}),t("div",{className:"gcse-search"})]})}const ct=m(at)`
  *,::before,::after{
...
`,vt=[He,Re,Oe,$e,We];function Et(e){return t(A,{children:vt.reduceRight((n,r)=>t(r,{children:n}),e.children)})}Be.render(t(S.StrictMode,{children:t(Et,{children:t(Fe,{theme:J,children:t(bt,{className:"app"})})})}),document.getElementById("root"));
